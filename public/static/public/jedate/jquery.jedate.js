...
;(function(root, factory) {
    ...
    jet.sortDate = function (time,format) {
        ...
        if(!numArr){
            var date = new Date(),tmsArr = [ date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds() ];
                return tmsArr;
    ...
    jet.dateStr = function (y, m, d) {
        return y.toString()+jet.digit(m).toString()+jet.digit(d).toString()
    }
    ...
    jedfn.createDaysHtml = function(ys, ms, opts, boxCell){
        ...
        var isfestival = function(y, m ,d) {
            var festivalStr;
            var datetext = jet.dateStr(y, m, d);
            if(opts.dataval == true && opts.data[datetext]){
                festivalStr = '<p><span class="solar">'+d+'</span><span class="lunar red">￥'+opts.data[datetext].adult+'</span></p>';
            }else if(opts.piaoval == true && opts.data[datetext]){
                festivalStr = '<p><span class="solar">'+d+'</span><span class="lunar red">￥'+opts.data[datetext].market+'</span></p>';
            }else if(opts.mode2val == true && opts.data[datetext]){
                festivalStr = '<p><span class="solar">'+d+'</span><span class="lunar red">￥'+opts.data[datetext][0].market+'</span></p>';
            }else if(opts.festival == true && lang.name == "cn"){
        ...
        for (var p = prevDaysNum - firstWeek + 1; p <= prevDaysNum; p++, count++) {
            ...
            if(ends.length > 0){
                ...
                if(opts.dataval == true || opts.piaoval == true || opts.mode2val == true){
                    var dateStr = jet.dateStr(prevM.y, prevM.m, p);
                    pCls = !dateOfLimit(prevM.y, prevM.m, p, false) && opts.data[dateStr] ? "other" : "disabled";
                }else {
        ...
        for(var b = 1; b <= daysNum; b++, count++){
            ...
            if(dateOfLimit(year, month, b, true)){
                if(ends.length > 0){
                    if(opts.dataval == true || opts.piaoval == true || opts.mode2val == true){
                        var dateStr = jet.dateStr(year, month, b);
                        if (jet.ymdDate == thisDate ){
                            bCls = jet.ymdDate == thisDate && opts.data[dateStr] ? "action" : (currOne != thisOne && thisOne == thisDate && opts.data[dateStr] ? "action" : "");
                        }else {
                            if(dayreg.test(b)){
                                bCls = ends[1] == true && opts.data[dateStr] ? "" : "disabled";
                            }else{
                                bCls = ends[1] == true && opts.data[dateStr] ? "disabled":"";
                            }
                        }
                    ...
                    if(opts.dataval== true || opts.piaoval == true || opts.mode2val == true){
                        var dateStr = jet.dateStr(year, month, b);
                        bCls = jet.ymdDate == thisDate && opts.data[dateStr] ? "action" : (currOne != thisOne && thisOne == thisDate && opts.data[dateStr] ? "action" : (opts.data[dateStr]?'':'disabled'));
                    }else {
                        bCls = jet.ymdDate == thisDate ? "action" : (currOne != thisOne && thisOne == thisDate ? "action" : "");
                    }
                }
        ...
        for(var n = 1, nlen = 42 - count; n <= nlen; n++){
            ...
            if(ends.length > 0){
                ...
                if(opts.dataval== true || opts.piaoval == true || opts.mode2val == true){
                    var dateStr = jet.dateStr(nextM.y, nextM.m, n);
                    nCls = !dateOfLimit(nextM.y, nextM.m, n, false) && opts.data[dateStr] ? 'other'  : "disabled";
                }else {
    ...
    jedfn.chooseDays = function(opts,boxCell) {
        ...
        if(opts.dataval && lang.name == "cn") {
            //鼠标进入提示框出现
            boxCell.find(".jedaul li").on("mouseover", function () {
                var _this = $(this), atlunar = jet.reMacth(_this.attr("data-ymd")),
                    tipDiv = $("<div/>",{"id":"jedatetipscon","class":"jedatetipscon"}),
                    lunar = jeLunar(parseInt(atlunar[0]), parseInt(atlunar[1]) - 1, parseInt(atlunar[2])),
                    datatext = lunar.solarYear.toString() + jet.digit(lunar.solarMonth) + jet.digit(lunar.solarDate);
                if(!opts.data[datatext]){
                    return false;
                }
                var tiphtml = '<p>' + lunar.solarYear + '\u5E74' + lunar.solarMonth + '\u6708' + lunar.solarDate + '\u65E5 ' + lunar.inWeekDays + '</p><p class="red">成人价： ￥' + opts.data[datatext].adult + '元' + '</p><p>儿童价： ￥' + opts.data[datatext].children + '元</p><p>库存： ' + opts.data[datatext].stock + '</p>';
                ...
                if($('body').find(".jedatetipscon").length > 0) $('body').find(".jedatetipscon").remove();
            });
            $('body').find(".jedatetipscon").on('click',function () {
                $(".jedatetipscon").remove();
            })
        }
        if(opts.piaoval && lang.name == "cn") {
            //鼠标进入提示框出现
            boxCell.find(".jedaul li").on("mouseover", function () {
                var _this = $(this), atlunar = jet.reMacth(_this.attr("data-ymd")),
                    tipDiv = $("<div/>",{"id":"jedatetipscon","class":"jedatetipscon"}),
                ...
                var tiphtml = '<p>' + lunar.solarYear + '\u5E74' + lunar.solarMonth + '\u6708' + lunar.solarDate + '\u65E5 ' + lunar.inWeekDays + '</p><p class="red">出售价： ￥' + opts.data[datatext].market + '元' + '</p><p>库存： ' + opts.data[datatext].stock + '</p>';
        ...
        if(opts.mode2val && lang.name == "cn") {
            ...
            boxCell.find(".jedaul li").on("mouseover", function () {
                ...
                var tiphtml = '<p>' + lunar.solarYear + '\u5E74' + lunar.solarMonth + '\u6708' + lunar.solarDate + '\u65E5 ' + lunar.inWeekDays + '</p>';
                    for(var x in opts.data[datatext]){
                        tiphtml += '<p class="red">'+opts.data[datatext][x].kind+'： ￥' + opts.data[datatext][x].market + '元' + ' | 库存：'+opts.data[datatext][x].stock+'</p>';
                    }
                var Fesjieri = (lunar.solarFestival || lunar.lunarFestival) != "" ? '' : "";
                var Fesjieqi = lunar.jieqi != "" ? '': "";
                var tiptext = (lunar.solarFestival || lunar.lunarFestival || lunar.jieqi) != "" ? (Fesjieri + Fesjieqi) : "";
