...
class Cart extends Base
{
    ...
    public function setCache($pet_name=''){
        ...
        $uid = empty($pet_names[1])?'':$pet_names[1];
        $where = ['order_id'=>0,'uid'=>$pet_names[1]];
        $count = $this->where($where)->value('count(*)');
        get_cache($name.'.count_'.$uid,$count);
    }
    // 添加购物车操作
    ...
    public function del_cart($where){
        ...
        if(empty($where['uid']) || empty($result)){
            return false;
        }
        $this->setCache('count_'.$where['uid']);
        return true;
    }
    // 加入收藏夹
